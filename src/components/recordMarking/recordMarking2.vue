...
<template>
  <div id="recordMarking2" style="height: 100%;">
    <view-box ref="viewBox" body-padding-top="46px" body-padding-bottom="60px">
      <x-header slot="header" class="header" :left-options="{backText: ''}">做题记录</x-header>
      <div>
      ...
      <!--题目列表-->
      <div>
        <router-link tag="div" :to="{name:'questionDetails',query:{type:3,qid:item.id}}" class="topicWrap" :key="index"
...
<style scoped>
  ...
  #recordMarking2 >>> .vux-loading-no-text .weui-toast {
    top: 50%;
    margin-top: -49px; /*no*/
  }
